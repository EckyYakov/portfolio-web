...
export class ContentLoader {
  ...
  async getBlogPosts(): Promise<BlogPost[]> {
    ...
    try {
      const response = await fetch(`${import.meta.env.BASE_URL}content/blog/manifest.json`);
  ...
  async getBlogPost(slug: string): Promise<BlogPost | null> {
    ...
    if (!post.content) {
      try {
        const response = await fetch(`${import.meta.env.BASE_URL}content/blog/posts/${slug}.md`);
